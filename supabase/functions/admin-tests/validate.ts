// Server-side schema-v2 validation for Reading tests. The client mirrors these
// checks for live feedback, but THIS is the source of truth: upsert writes
// nothing unless the errors list is empty.
...
// Pass `partNumber` (1–5) to validate a SINGLE-PART test: content.parts must
// then hold exactly that one canonical part, judged by the same per-part rules
// as in the full paper; the 35-question total check is skipped.
// deno-lint-ignore no-explicit-any
export function validateReadingTest(content: any, partNumber?: number): string[] {
  const errors: string[] = []
  ...
  content.parts.forEach((part: any, index: number) => {
    ...
    if (partNo === 1 || partNo === 4 || partNo === 5) {
      if (typeof part.passage?.html !== 'string' || !part.passage.html.trim()) {
        err(`${label}: passage text (html) is required.`)
      }
    }
    if (partNo === 2) {
      if (!Array.isArray(part.optionPool) || part.optionPool.length !== 10) {
        err(`${label}: optionPool must have exactly 10 statements A–J (got ${part.optionPool?.length ?? 0}).`)
      }
    }
    if (partNo === 3) {
      const poolLen = part.optionPool?.length ?? 0
      if (!Array.isArray(part.optionPool) || poolLen < 8 || poolLen > 9) {
        err(`${label}: optionPool must have 8 or 9 headings (got ${poolLen}).`)
      }
      if (!Array.isArray(part.passage?.paragraphs) || part.passage.paragraphs.length !== 6) {
        err(`${label}: passage must have exactly 6 paragraphs (got ${part.passage?.paragraphs?.length ?? 0}).`)
      } else {
        part.passage.paragraphs.forEach((p: any, i: number) => {
          if (typeof p?.html !== 'string' || !p.html.trim()) err(`${label}: paragraph ${i + 1} text is empty.`)
          if (typeof p?.label !== 'string' || !p.label.trim()) err(`${label}: paragraph ${i + 1} label is empty.`)
        })
      }
    }
    ...
    // Gap markers in this part's passage html(s)
    const htmlSources = [part.passage?.html ?? '', ...((part.passage?.paragraphs ?? []).map((p: any) => p?.html ?? ''))]
    const markerIds = new Set<string>()
    for (const html of htmlSources) {
      for (const m of String(html).matchAll(GAP_MARKER_RE)) markerIds.add(m[1])
    }
    const partItemIds = new Set(items.map((i: any) => i?.id))
    ...
    items.forEach((item: any, i: number) => {
      ...
      if (!ALLOWED_TYPES_PER_PART[partNo - 1].includes(item.type)) {
        err(`${qLabel}: type '${item.type}' is not allowed in ${label} (allowed: ${ALLOWED_TYPES_PER_PART[partNo - 1].join(', ')}).`)
      }

      switch (item.type) {
        case 'gap': {
          const answers = Array.isArray(item.answer) ? item.answer.filter((a: any) => typeof a === 'string' && a.trim()) : []
        ...
        case 'tfng': {
          ...
          if (!['true', 'false', 'not_given'].includes(item.answer)) {
            err(`${qLabel}: tfng answer must be true, false or not_given (got '${item.answer}').`)
          }
